<template>
  <div
    ...
    :class="[
      ...
      active ? 'border-red-400 bg-gradient-to-br from-red-50 to-red-100 hover:shadow-lg' : 'border-gray-100 bg-gradient-to-br from-gray-50 to-gray-100',
      disabled ? 'opacity-50' : ''
    ]"
    @click="$emit('click')"
  >
    <span :class="[
      ...
      label === 'Return Cancelled' || label == 'Return Declined' || label == 'Request Cancelled' || label == 'Request Declined' ? 'bg-red-200 text-red-700' : (active ? 'bg-red-200 text-red-700' : 'bg-gray-200 text-gray-400')
    ]">
      <i v-if="icon" :class="icon + ' text-sm'" />
    </span>
    <span :class="[
      'font-semibold text-xs mb-1',
      label === 'Return Cancelled' || label == 'Return Declined' || label == 'Request Cancelled' || label == 'Request Declined' ? 'text-red-700' : (active ? 'text-red-700' : 'text-gray-400')
    ]">{{ label }}</span>
