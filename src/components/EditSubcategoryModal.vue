  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
    <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
      <div class="flex items-center mb-4">
        ...
        <h2 class="text-lg font-bold text-red-700">Edit Subcategory Name</h2>
        <button class="ml-auto text-gray-400 hover:text-red-700" @click="$emit('close')">
      ...
      <form @submit.prevent="submit">
        <div class="mb-4">
          ...
          <input v-model="name" type="text" class="w-full border border-red-200 rounded px-3 py-2 text-xs" required />
        </div>
