  ...
  <Dialog :visible="open" modal :style="{ width: '32rem', borderRadius: '1rem' }" @click="$emit('close')">
    <template #header>
      <div class="flex items-center gap-3">
        ...
        <div>
          <div class="text-lg font-bold text-red-700">{{ props.subcategoryName || 'Subcategory' }}</div>
          <div class="text-sm text-gray-500">Category: <span class="font-semibold text-red-600">{{ props.categoryName || '-' }}</span></div>
        </div>
      </div>
    </template>
    <div v-if="subcategoryName">
      <div class="mb-4 mt-2">
        <span class="font-bold text-red-700">Materials</span>
      </div>
      <ul class="mt-2 rounded-lg overflow-hidden border border-red-100 bg-white shadow">
        <li v-for="mat in filteredMaterials" :key="mat.id" class="py-2 px-4 border-b last:border-b-0 flex justify-between items-center hover:bg-red-50 transition">
    ...
    <div v-else class="flex flex-col items-center justify-center py-8">
      <i class="fas fa-spinner fa-spin text-2xl text-red-400 mb-2"></i>
